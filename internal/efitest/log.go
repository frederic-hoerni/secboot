...
func maybeMeasureThunderboltSecurityLevel0Event(c *C, builder *logBuilder, opts *LogOptions) {
	if opts.ThunderboltSecurityLevel0&ThunderboltSecurityLevel0 > 0 {
		var data tcglog.EventData
...
// ThunderboltSecurityLevel0EventFlags specifies whether there should be a EV_EFI_ACTION
// "Security Level is Downgraded to 0" event in PCR7.
type ThunderboltSecurityLevel0EventFlags int

const (
	// ThunderboltSecurityLevel0 indicates that a EV_EFI_ACTION event should be included,
	// as it would be on systems with a "Security Level is Downgraded to 0" event.
	ThunderboltSecurityLevel0 ThunderboltSecurityLevel0EventFlags = 1 << iota
)
